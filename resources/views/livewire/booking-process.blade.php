...
<div>
    ...
    {{-- Mensagem se o usuário já tiver um agendamento ativo --}}
    @if ($userHasActiveUpcomingAppointment)
        <div class="mb-6 p-4 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 dark:bg-yellow-900 dark:border-yellow-700 dark:text-yellow-300">
            ...
            <p>Você já possui um agendamento futuro ativo. Para marcar um novo, por favor, cancele o existente ou aguarde sua realização.</p>
    ...
    {{-- Envolve o formulário em um fieldset para desabilitar tudo de uma vez, se desejado --}}
    {{-- <fieldset @if($userHasActiveUpcomingAppointment) disabled @endif> --}}

    {{-- ETAPA 1: Selecionar Serviço --}}
    <div class="mb-6 @if($userHasActiveUpcomingAppointment) opacity-50 pointer-events-none @endif">
        <label for="service" class="block text-sm font-medium text-gray-700 dark:text-gray-300">1. Escolha o Serviço:</label>
        <select wire:model.lazy="selectedServiceId" id="service" name="service"
                ...
                @if($userHasActiveUpcomingAppointment) disabled @endif>
    ...
    @if ($selectedServiceId)
        <div class="mb-6 @if($userHasActiveUpcomingAppointment) opacity-50 pointer-events-none @endif">
            <label for="date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">2. Escolha a Data:</label>
    ...
    @if ($selectedServiceId && $selectedDate) {{-- Mostra esta seção mesmo se availableTimeSlots estiver vazio para dar feedback --}}
        <div class="mb-6 @if($userHasActiveUpcomingAppointment) opacity-50 @endif">
            <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300">3. Escolha um Horário para {{ $selectedDate ? \Carbon\Carbon::parse($selectedDate)->format('d/m/Y') : '' }}:</h3>
                    ...
                    @foreach ($availableTimeSlots as $slot)
                        <button type="button" wire:click="selectTimeSlot('{{ $slot }}')"
                                class="py-2 px-3 border rounded-md text-sm font-medium
                                       ...
                                       @if($userHasActiveUpcomingAppointment) opacity-50 pointer-events-none cursor-not-allowed @endif"
                                @if($userHasActiveUpcomingAppointment) disabled @endif>
                            {{ $slot }}
                        </button>
                    @endforeach
                </div>
            @elseif (!$userHasActiveUpcomingAppointment) {{-- Só mostra "nenhum horário" se o form não estiver desabilitado --}}
        ...
        <div class="mt-8">
            <button wire:click="bookAppointment" type="button"
                    class="w-full inline-flex items-center justify-center px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500
                           @if($userHasActiveUpcomingAppointment) opacity-50 cursor-not-allowed @endif"
                    @if($userHasActiveUpcomingAppointment) disabled @endif
                    wire:loading.attr="disabled" wire:loading.class="opacity-50"> {{-- Desabilita durante o carregamento do agendamento --}}
